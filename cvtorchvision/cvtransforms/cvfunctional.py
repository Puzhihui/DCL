...
def swap(img, crop_size):
    ...
    widthcut, highcut = img.shape[1], img.shape[0]
    # img = img.crop((10, 10, widthcut - 10, highcut - 10))
    img = crop(img, 10, 10, widthcut-20, highcut - 20)
    images = crop_image(img, crop_size)
    pro = 5
    if pro >= 5:
        tmpx = []
        tmpy = []
        ...
        k = 1
        RAN = 2
        for i in range(crop_size[1] * crop_size[0]):
        ...
        width, high = img.shape[1], img.shape[0]
        iw = int(width / crop_size[0])
        ih = int(high / crop_size[1])
        toImage = np.zeros((ih * crop_size[1], iw * crop_size[0], 3), dtype=np.uint8)
        x = 0
        y = 0
        for i in random_im:
            i = cv2.resize(i, dsize=(iw, ih), interpolation=INTER_MODE["BILINEAR"])
            toImage[y * ih:y * ih+ih, x * iw:x * iw+iw] = i
            x += 1
            if x == crop_size[0]:
                x = 0
                y += 1
        ...
        toImage = img
    toImage = cv2.resize(toImage, dsize=(widthcut, highcut), interpolation=INTER_MODE["BILINEAR"])
    return toImage
